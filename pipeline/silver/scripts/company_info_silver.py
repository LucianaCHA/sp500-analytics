...
def transform_data(
    ...
    df = df.rename(
        columns={"company_ticker": "symbol", "id": "company_id"}, errors="ignore"
    )
    ...
    for col in text_dim_cols:
        if col in df.columns:
            ...
            df[col] = (
                ...
                .astype(str)
                .str.strip()
                .replace("nan", "Unknown", regex=False)
                .str.title()
            )
    ...
    print(
        f"✔ Normalización completa. DataFrame de Hechos (INFO) listo. Filas: {len(df_fact_table)}"
    )
...
def load_to_s3(df_fact_table: pd.DataFrame):
    ...
    try:
        ...
        s3_client.put_object(
            Bucket=SILVER_BUCKET_NAME, Key=S3_KEY_PATH, Body=parquet_buffer.read()
        )
...
def load_to_rds(
    ...
    try:
        with engine.begin() as connection:
            ...
            # PRIMARY KEYs
            connection.execute(
                text(f"ALTER TABLE {DIM_SECTOR_NAME} ADD PRIMARY KEY (sector_id);")
            )
            connection.execute(
                text(f"ALTER TABLE {DIM_INDUSTRY_NAME} ADD PRIMARY KEY (industry_id);")
            )
            connection.execute(
                text(f"ALTER TABLE {DIM_LOCATION_NAME} ADD PRIMARY KEY (location_id);")
            )
            connection.execute(
                text(f"ALTER TABLE {DIM_EXCHANGE_NAME} ADD PRIMARY KEY (exchange_id);")
            )
            connection.execute(
                text(f"ALTER TABLE {FACT_TABLE_NAME} ADD PRIMARY KEY (company_id);")
            )

            # FOREIGN KEYs (FKs)
            connection.execute(
                text(
                    f"ALTER TABLE {FACT_TABLE_NAME} ADD FOREIGN KEY (sector_id) REFERENCES {DIM_SECTOR_NAME} (sector_id);"
                )
            )
            connection.execute(
                text(
                    f"ALTER TABLE {FACT_TABLE_NAME} ADD FOREIGN KEY (industry_id) REFERENCES {DIM_INDUSTRY_NAME} (industry_id);"
                )
            )
            connection.execute(
                text(
                    f"ALTER TABLE {FACT_TABLE_NAME} ADD FOREIGN KEY (location_id) REFERENCES {DIM_LOCATION_NAME} (location_id);"
                )
            )
            connection.execute(
                text(
                    f"ALTER TABLE {FACT_TABLE_NAME} ADD FOREIGN KEY (exchange_id) REFERENCES {DIM_EXCHANGE_NAME} (exchange_id);"
                )
            )
...
def main():
    ...
    df_fact_table, df_dim_sector, df_dim_industry, df_dim_location, df_dim_exchange = (
        transform_data(df_raw)
    )
